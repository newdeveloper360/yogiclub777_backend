...
$(function () {
    ...
    // Function to save sidebar color preference to local storage
    function saveSidebarColorPreference(color) {
        localStorage.setItem("sidebarColorPreference", color);
    }

    // Function to save layout color preference to local storage
    function saveLayoutColorPreference(color) {
        localStorage.setItem("layoutColorPreference", color);
    }

    // Apply saved preferences on page load
    $(document).ready(function () {
        var layoutColorPreference = localStorage.getItem(
            "layoutColorPreference"
        );

        if (layoutColorPreference) {
            if (layoutColorPreference == 1) {
                $("body").removeClass();
                $("body").addClass("light");
                $("body").addClass("light-sidebar");
                $("body").addClass("theme-white");
 
                $(".selectgroup-input-radio select-layout[value|='1']").prop("checked", true);
            } else {
                $("body").removeClass();
                $("body").addClass("dark");
                $("body").addClass("dark-sidebar");
                $("body").addClass("theme-black"); 
                $(".selectgroup-input-radio select-layout[value|='2']").prop("checked", true);
            }
        }
        if (layoutColorPreference) {
            $(".select-layout[value='" + layoutColorPreference + "']").prop(
                "checked",
                true
            );
        } else {
            $(".select-layout[value='1']").prop("checked", true); // Default layout color
        }
    });
    ...
    $(".choose-theme li").on("click", function () {
        ...
        saveThemePreference("theme-" + selectedTheme);

        // Remove and add classes to apply the selected theme
        $("body")
        ...
        // Update the active state of the theme options
    ...
    $(".layout-color input:radio").change(function () {
        ...
        // Save the selected layout color preference to local storage
        saveLayoutColorPreference(selectedColor);

        // Remove and add classes to apply the selected layout color
        $("body")
            .removeClass()
            .addClass(
                selectedColor === "1"
